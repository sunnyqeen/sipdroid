...
Flag   g729a_enc_init     (void *encState)
{
  ...
  g729a_Init_Pre_Process(&state->preProcessState);
  g729a_Init_Coder_ld8a(&state->encoderState);
...
#endif
{
  ...
  g729a_Pre_Process(&state->preProcessState, pcm, state->encoderState.new_speech, L_FRAME);

  g729a_Coder_ld8a(&state->encoderState, prm);
