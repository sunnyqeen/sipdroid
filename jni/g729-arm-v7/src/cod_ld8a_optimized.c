 ...
 *------------------------------------------------------------------------*/
  ...
  *----------------------------------------------------------------------*/
...

  {
    ...
    for(i=1; i<=M; i++)    /* Ap1[i] = Ap[i] - 0.7 * Ap[i-1]; */
    {
       ...
       //Aq1[i] = sub(Aq[i], mult(Aq[i-1], 22938));	
       Ap1[i] = Ap[i] - mult(Ap[i-1], 22938);
       Aq1[i] = Aq[i] - mult(Aq[i-1], 22938);	
    }
    Syn_filt(Ap1, &state->exc[0], &state->wsp[0], L_SUBFR, state->mem_w, 1);
    Syn_filt(Aq1, &state->exc[L_SUBFR], &state->wsp[L_SUBFR], L_SUBFR, state->mem_w, 1);
  }
  ...
  *------------------------------------------------------------------------*/
  ...
  for (i0 = 0;  i0 < L_FRAME/L_SUBFR; i0++)
  {
	i_subfr = i0 * L_SUBFR;
    /*---------------------------------------------------------------*
    ...
    *------------------------------------------------------*/
    ...
    for (i = 0; i < L_SUBFR;  i++)
    {
      ...
      //L_temp = L_mult(state->exc[i+i_subfr], gain_pit);
