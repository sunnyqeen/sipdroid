...
Flag   g729a_dec_init (void *decState)
{
  ...
  g729a_Init_Decod_ld8a(&state->decoderState);
  g729a_Init_Post_Filter(&state->postFilterState);
  g729a_Init_Post_Process(&state->postProcessState);
...
#endif
{
  ...
  g729a_Decod_ld8a(&state->decoderState, parm, state->synth, Az_dec, T2, bad_lsf);

  g729a_Post_Filter(&state->postFilterState, state->synth, Az_dec, T2);        /* Post-filter */

  g729a_Post_Process(&state->postProcessState, state->synth, pcm, L_FRAME);
}
